    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {questions.map((question) => (
                <div 
                    key={question.id}
                    className="group relative bg-white/70 backdrop-blur-xl rounded-2xl p-6 border border-white/20 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                >
                    {/* Status Indicator */}
                    <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-medium border ${getStatusBadge(question.is_correct)}`}>
                        {question.is_correct}
                    </div>
                    ...
                    <div className="relative z-10">
                        <h3 
                            ...
                            className="text-slate-800 font-semibold mb-4 line-clamp-3 group-hover:text-indigo-600 transition-colors leading-relaxed">
                        {question.problem}
                        </h3>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-200">
                            ...
                            <div key={index} className="text-slate-600 text-sm leading-relaxed bg-slate-50/50 rounded-lg p-3">
                                {answer.split('\n').map((line, i) => (
                        ...
                        {/* Quick Actions */}
                        <div className="flex justify-between items-center mt-4 pt-4 border-t border-slate-200 opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className="text-xs text-slate-500">Question #{question.id}</span>
