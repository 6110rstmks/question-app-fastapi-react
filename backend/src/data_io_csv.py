...
def export_data_to_csv(db: AsyncSession, output_dir: str):
    """
    ...
    with open(subcategories_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        subcategories = subcategory_repository.get_all()
    ...
    with open(category_question_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        category_questions = category_question_repository.get_all()
    ...
    with open(subcategory_question_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        subcategory_questions = subcategory_question_repository.get_all()
...
# ↓テストしていないので一旦コメントアウト。リポジトリパターンに置き換えと、テストコード書いたら復活させる。
