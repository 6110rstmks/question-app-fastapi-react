...
# @router.get("/question_id/{question_id}", response_model=SubcategoryQuestionResponse, status_code=status.HTTP_200_OK)
# async def find_subcategoryquestion_by_question_id(
#     db: DbDependency,
#     question_id: int = Path(gt=0),
# ):
#     return subcategory_question_cruds.find_subcategoryquestion_by_question_id(db, question_id)

@router.get("/question_id/{question_id}", response_model=list[SubcategoryQuestionResponse], status_code=status.HTTP_200_OK)
