...
class ProblemFetch(BaseModel):
    ...
    problem_count: int = Field(gt=0, example=10, description="出題する問題数")
    category_ids: Optional[List[int]] = Field(default=None, example=[38, 40], description="出題するカテゴリーID群")
    subcategory_ids: Optional[List[int]] = Field(default=None, example=[1, 2], description="出題するサブカテゴリーID群")
